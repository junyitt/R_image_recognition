...
# Parameters --------------------------------------------------------------
pretrained_model_id <- "Identity"
path <- getwd()

data_path <- file.path(path, "Data/_TRAIN_Combined_IdentityData")
test_path <- file.path(path, "Data/_TEST_Combined_IdentityData")

unseen_train_data_path <- file.path(path, "Data/_TRAIN_UNSEEN_LFW")
unseen_test_data_path <- file.path(path, "Data/_TEST_UNSEEN_LFW")
...
dir.create(file.path(path, "Cache"), showWarnings = FALSE)
...
save(train_features, file = file.path(path, "Cache/train_features.rda"))
save(train_label, file = file.path(path, "Cache/train_label.rda"))
...
train_label_sample <- unique(all_train_label)
train_label_sample <- train_label_sample[1:7]
...
unseen_label_sample <- as.vector(na.omit(unseen_label_sample))
